...
namespace CRPParser
{
    public class SettingsFile
    {
        ...
        public string cloudName
        {
            ...
            {
                this.m_PathName = PathUtils.AddExtension( value, GameSettings.extension );
                this.m_UseSteamCloud = true;
            }
        }

        internal Stream CreateReadStream()
        {
            if ( this.m_UseSteamCloud )
            {
                return new MemoryStream( Steam.cloud.ReadAllBytes( this.pathName ) );
            }
        ...
        internal Stream CreateWriteStream()
        {
            if ( this.m_UseSteamCloud )
            {
                return new SteamCloudStream( this.pathName );
            }
        ...
        public bool IsValid()
        {
            ...
            return Steam.cloud.Exists( this.pathName );
        }
        ...
        public void Delete()
        {
            if ( this.IsValid() )
            {
                if ( this.m_UseSteamCloud )
                {
                    Steam.cloud.Delete( this.pathName );
                    return;
                }
        ...
        private void Serialize( Stream stream )
        {
            ...
            using ( BinaryWriter binaryWriter = new BinaryWriter( stream ) )
            {
                ...
                foreach ( KeyValuePair<string, bool> current2 in dictionary2 )
                {
                    binaryWriter.Write( current2.Key );
                    binaryWriter.Write( current2.Value );
                }
                binaryWriter.Write( dictionary3.Count );
                foreach ( KeyValuePair<string, float> current3 in dictionary3 )
                {
                    binaryWriter.Write( current3.Key );
                    binaryWriter.Write( current3.Value );
                }
                binaryWriter.Write( dictionary4.Count );
                foreach ( KeyValuePair<string, string> current4 in dictionary4 )
                {
                    binaryWriter.Write( current4.Key );
                    binaryWriter.Write( current4.Value );
                }
                binaryWriter.Write( dictionary5.Count );
                foreach ( KeyValuePair<string, InputKey> current5 in dictionary5 )
                {
                    binaryWriter.Write( current5.Key );
                    binaryWriter.Write( current5.Value );
                }
                binaryWriter.Flush();
            }
        }
        ...
        internal void Save()
        {
            if ( !this.dontSave && !string.IsNullOrEmpty( this.pathName ) )
            {
                ...
                CODebugBase<InternalLogChannel>.Log( InternalLogChannel.Settings, "Saving " + this.m_PathName );
            }
        }
        ...
        internal void SetValue( string name, string val )
        {
            ...
            {
                CODebugBase<InternalLogChannel>.VerboseLog( InternalLogChannel.Settings, "Setting " + name + " updated to " + val );
        ...
        internal void SetValue( string name, bool val )
        {
            ...
            {
                CODebugBase<InternalLogChannel>.VerboseLog( InternalLogChannel.Settings, string.Concat( new object[]
                {
                    "Setting ",
                    name,
                    " updated to ",
        ...
        {
            float num;
            if ( !this.m_SettingsFloatValues.TryGetValue( name, out num ) || !Mathf.Approximately( this.m_SettingsFloatValues[name], val ) )
            {
                CODebugBase<InternalLogChannel>.VerboseLog( InternalLogChannel.Settings, string.Concat( new object[]
                {
                    "Setting ",
